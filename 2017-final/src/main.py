...
def process_router(mapping,router_budget,maxSeconds=None):
    ...
    def write_router_output(file):
        ...
        for i in range(rowsN):
            for j in range(columsN):
                if(int(pulp.value(mapping_router[(i,j)]))==0):
                    f.write(convert(mapping[i][j]))
                else: f.write("r")
            f.write("\n")
        f.close()
    def write_target_output(file):
        '''
        Ecrit les cellules qui capte (représenté par un 1 )
        '''
        f = open(file,"a+")
        for i in range(rowsN):
            for j in range(columsN):
                if(int(pulp.value(targetcell[(i,j)]))==1):
                     f.write("1")
                else: f.write(convert(mapping[i][j]))
            f.write("\n")
        f.close()
    ...
    for i in range(rowsN):
        for j in range(columsN):
            if(int(pulp.value(targetcell[(i,j)]))==1):
                score+=1
    # Write output
    write_router_output(CHARLESTON_ROUTER_OUT)
    write_target_output(CHARLESTON_TARGET_OUT)


    return (prob,score)

# Remet les fichiers output à 0
reset_output_file(CHARLESTON_ROUTER_OUT)
reset_output_file(CHARLESTON_TARGET_OUT)
section = 5
crossing = int(rowsN/section)
budgetPerStep = int(router_budget/crossing)
print("----Starting resolution ----")
for x in range(0,crossing):
    (prob,score) = process_router(mapping[x*section:x*section+section],budgetPerStep,10)
    print("step %d terminated,  score: %d" % (x, score))
